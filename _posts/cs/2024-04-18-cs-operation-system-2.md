...
프로세스와 쓰레드의 역할의 차이를 잘 알아야 한다. 먼저, 프로세스는 실행의 단위이며 쓰레드는 작업의 흐름이란 것을 잘 기억하자.
프로세스는 하나의 프로그램을 의미한다. 그래서 실행의 단위라고 하며, 프로세스는 CPU가 하나만 실행할 수 있다.
가장 큰 특징은 스레드와 비교하면 프로세스는 독립된 자원 공간을 사용한다.
...
중요한 특징은 쓰레드는 다른 쓰레드와 자원을 공유하는 데이터가 많아 프로세스와 비교했을 때 오버헤드가 덜 일어난다.

> 오버헤드 : 컨텍스트 스위칭이 일어날 때 CPU가 잠시동안 작업을 멈추는 시간을 말한다.

#### 사용자 레벨 쓰레드, 커널 레벨 쓰레드  
쓰레드도 커널 모드와 사용자 모드로 나뉜다.  
**사용자 레벨 쓰레드**는 사용자가 라이브러리를 이용해 생성 및 관리한다.  
**커널 레벨 쓰레드**는 커널이 쓰레드를 생성 및 관리한다.
