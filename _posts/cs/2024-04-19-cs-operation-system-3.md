...
이전 글에서도 말했듯이 CPU는 하나의 프로세스만 처리할 수 있다. 
그렇기 때문에 다른 프로세스를 실행하려면 콘텍스트 스위칭이 이루어져야 한다.
...
위 과정에서 추가적으로 1~2, 5~6 과정에서는 P2가 유휴 상태(idle)상태가 되고, 전 과정에서 P1은 유휴 상태가 된다. 그리고 2~3과정에서는 P1, P2 유휴 상태이고, CPU는 아무 일도 못하게 된다.
이러한 시간과 메모리가 소요될 때를 **오버헤드**라고 한다.
...
**경쟁 상태(Race Condition)** : 프로세스 또는 쓰레드에서 하나의 공유 자원에 접근 할 때 접근 순서에 따라 결과 값이 달라 지는 경우를 경쟁 상태라고 한다.
...
`number`라는 변수를 증가시키는 함수가 있다고 하자. 원래의 정상적인 출력 값을 생각하면 1, 2가 될 것이다. 
하지만 만약에 동기화가 이뤄지지 않는다면 1, 1이 출력 되는 문제가 발생한다. 이러한 문제를 동기화하여 해결해야 한다.
그리고 이러한 number 영역에 접근하는 것을 임계 영역이라고 하는데 임계 영역은 다음과 같다.

- 임계 영역 : 공유 자원에 접근할 수 있고 접근 순서에 따라 달라지는 코드 영역

즉, 임계 영역에서 일어나는 경쟁 상태를 동기화하여 데이터에 일관성을 유지해야 한다.
...
3. **한정된 대기(bounded waiting)** : 임계 영역 접근 요청 후 대기일 때 **무한정 대기**하지 않게 한다.

### 상호배제 기법
#### 뮤텍스

뮤텍스는 락을 가진 프로세스만 접근할 수 있게 하는 기법이다.
임계 영역에 먼저 접근한 프로세스가 락을 걸면 다른 프로세스들은 해당 프로세스가 락을 해제하기 전까지 대기 큐에서 대기한다.
그리고 접근할 수 있는 상태가 되면 대기 큐에서 기다리단 프로세스 하나가 자원에 접근하게 된다.

#### 세마포어

공유 자원에 접근할 수 있는 프로세스의 수를 정해 접근을 제어하는 기법이다. 
접근한 프로세스가 해제를 하면 다른 프로세스가 접근할 수 있도록 신호를 보낸다.

#### 스핀 락
스핀 락은 뮤텍스와 같이 하나만 접근할 수 있지만 다른 점은 스핀 락은 대기 큐가 없고 무한정 반복하면 대기한다.

### 교착 상태
교착 상태는 상호배재 기법을 사용하다 발생한 문제를 말한다. 
2개 이상의 프로세스가 서로 자원에 접근하다가 2개 모두 대기하게 되어 무한정 대기하는 문제가 발생한다.
다다음과 같은 4가지 조건을 모두 채우면 문제가 생긴다.
...
교착 상태를 방지하려면 4가지 조건 중 한 가지만 제거하면 해결 된다.

### 쓰레드 안전

쓰레드 안전은 멀티 쓰레드 환경에서 하나의 변수, 함수, 객체 쓰레드 여러 개가 동시에 접근해도 문제가 없음을 의미 한다.
쓰레드가 안전하지 않은 경우는 위에 프로세스에서 봤던 동기화가 이루어지지 않았을 경우와 똑같다.
