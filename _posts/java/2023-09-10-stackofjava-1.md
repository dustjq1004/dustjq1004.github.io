...
Stack을 보면 Vector를 상속받았고 대부분이 vector에 의존적이라 볼 수 있다. 사실 의존적인 것은 둘째치고 상속 관계에서 문제가 생긴다.
하위 클래스는 상위 클래스로 형변환이 가능하단 것을 알고 있을 것이다. Vector는 다양한 메서드를 가지고 있다.
